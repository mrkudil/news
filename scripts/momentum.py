#!/usr/bin/env python3
# -----------------------------------------------------------------------------
# LIVE PRICE PUBLISH PATCH
# Generated by M365 Copilot on 2026-05-20.
# Publishes live price_current/_price_current for live-only pivot.py and writes
# matching rates.json for dashboard NOW price alignment.
# -----------------------------------------------------------------------------
# UNIFIED MOMENTUM + PIVOT TELEGRAM PATCH
# Adds institutional-style Telegram alerts combining H1 momentum context with
# live pivot.py structure, ATR confirmation, and pivot-zone trade plan.
# -----------------------------------------------------------------------------
"""
momentum.py
...
Files written into MOMENTUM_DATA_DIR (default: script directory):
  ...
  rates.json (optional, live prices for dashboard)
  macro_components.json live price fields (optional merge)
...
# ---------------------------------------------------------------------------
# Live-price publishing for pivot.py / pivots.html
# ---------------------------------------------------------------------------
PUBLISH_LIVE_CONTEXT = _env_bool("MOMENTUM_PUBLISH_LIVE_CONTEXT", True)
PUBLISH_RATES_JSON = _env_bool("MOMENTUM_PUBLISH_RATES_JSON", True)
LIVE_PRICE_MAX_AGE_SEC = _env_positive_int("MOMENTUM_LIVE_PRICE_MAX_AGE", 180, minimum=1)
H1_ATR_PERIOD = _env_positive_int("MOMENTUM_H1_ATR_PERIOD", 14, minimum=2)
LIVE_CONTEXT_FILE = Path(os.environ.get(
    "MOMENTUM_LIVE_CONTEXT_FILE",
    os.environ.get("MACRO_COMPONENTS_FILE", os.environ.get("MACRO_COMPONENT_CACHE", str(SCRAPER_COMPONENT_FILE))),
))
RATES_FILE = Path(os.environ.get("RATES_FILE") or os.environ.get("MOMENTUM_RATES_FILE") or str(BASE_DIR / "rates.json"))

# ---------------------------------------------------------------------------
# Unified momentum + pivot execution Telegram alerts
# ---------------------------------------------------------------------------
UNIFIED_EXECUTION_ALERTS_ENABLED = _env_bool("MOMENTUM_UNIFIED_EXECUTION_ALERTS", True)
UNIFIED_EXECUTION_REQUIRE_PIVOT_LIVE = _env_bool("MOMENTUM_UNIFIED_REQUIRE_PIVOT_LIVE", True)
UNIFIED_EXECUTION_REQUIRE_ATR_CONFIRMED = _env_bool("MOMENTUM_UNIFIED_REQUIRE_ATR_CONFIRMED", False)
UNIFIED_EXECUTION_MIN_RR = _env_float("MOMENTUM_UNIFIED_MIN_RR", 1.20, minimum=0.0)
UNIFIED_EXECUTION_ALERT_TTL_HOURS = _env_positive_int("MOMENTUM_UNIFIED_ALERT_TTL_HOURS", 12, minimum=1)
PIVOT_LEVELS_FILE = Path(os.environ.get("PIVOT_LEVELS_FILE") or os.environ.get("MOMENTUM_PIVOT_LEVELS_FILE") or str(BASE_DIR / "pivot_levels.json"))
...
# ---------------------------------------------------------------------------
# Live context / rates publishing helpers
# ---------------------------------------------------------------------------

def _pair_label(pair: str) -> str:
    p = _normalize_pair_code(pair)
    return f"{p[:3].upper()}/{p[3:].upper()}" if len(p) == 6 else p.upper()


def _price_digits(pair: str) -> int:
    return 2 if "xau" in _normalize_pair_code(pair) else 6


def _round_price(pair: str, value: Any) -> Optional[float]:
    try:
        if value is None:
            return None
...
def _h1_atr_proxy_from_rows(rows: List[Dict[str, Any]], period: int = H1_ATR_PERIOD) -> Optional[float]:
    """Average absolute close-to-close movement over latest completed H1 closes."""
...
def _atomic_merge_json(path: Path, patch: Dict[str, Any]) -> bool:
    """Atomically merge top-level JSON keys without clobbering nested dicts like sources."""
    try:
        ...
        log.warning("momentum: live context publish failed path=%s err=%s", path, exc)
        return False


def _publish_rates_json(pairs: List[str], current_prices: Dict[str, Optional[float]], current_sources: Dict[str, str], ts_iso: str) -> bool:
    if not PUBLISH_RATES_JSON:
        return False
    rates: List[Dict[str, Any]] = []
    for pair in pairs:
        price = _round_price(pair, current_prices.get(pair))
        if price is None:
            continue
        rates.append({
            "pair": pair,
            ...
            "rate": price,
            "source": current_sources.get(pair, SOURCE_UNAVAILABLE),
            "ts": ts_iso,
        })
    payload = {"fetched_at": ts_iso, "generated_at": ts_iso, "source": "momentum_live_context", "rates": rates}
    try:
        RATES_FILE.parent.mkdir(parents=True, exist_ok=True)
        tmp = RATES_FILE.with_suffix(RATES_FILE.suffix + ".tmp")
        with tmp.open("w", encoding="utf-8") as f:
            json.dump(payload, f, indent=2)
        os.replace(tmp, RATES_FILE)
        try:
            os.chmod(RATES_FILE, 0o644)
        except OSError:
            pass
        return True
    except Exception as exc:
        log.warning("momentum: rates.json publish failed path=%s err=%s", RATES_FILE, exc)
        return False


def _publish_live_context(
    pairs: List[str],
    histories: Dict[str, List[Dict[str, Any]]],
    current_prices: Dict[str, Optional[float]],
    current_sources: Dict[str, str],
    h1_ema_state: Dict[str, Dict[str, Any]],
) -> Dict[str, Dict[str, Any]]:
    """Publish live-only price context for pivot.py and rates.json for dashboard NOW."""
    if not PUBLISH_LIVE_CONTEXT:
        return {p: {"ok": False, "published_to_macro_components": False, "published_to_rates": False, "reason": "disabled"} for p in pairs}
    ts_iso = _iso_z(_now_utc())
    price_current: Dict[str, float] = {}
    price_current_ts: Dict[str, str] = {}
    price_current_source: Dict[str, str] = {}
    price_session_ohlc: Dict[str, Dict[str, Any]] = {}
    h1_atr: Dict[str, float] = {}
    h1_rsi_state: Dict[str, Dict[str, Any]] = {}
    live_health: Dict[str, Dict[str, Any]] = {}

    for pair in pairs:
        price = _round_price(pair, current_prices.get(pair))
        source = current_sources.get(pair, SOURCE_UNAVAILABLE)
        atr_val = _h1_atr_proxy_from_rows(histories.get(pair, []), H1_ATR_PERIOD)
        if atr_val is not None:
            h1_atr[pair] = round(float(atr_val), _price_digits(pair))
        ema_state = h1_ema_state.get(pair, {}) if isinstance(h1_ema_state, dict) else {}
        if isinstance(ema_state, dict):
            h1_rsi_state[pair] = {
                "pair": pair,
                "timeframe": "H1",
                "period": int(ema_state.get("h1_rsi_period", MOMENTUM_RSI_PERIOD) or MOMENTUM_RSI_PERIOD),
                "value": ema_state.get("h1_rsi"),
                "state": ema_state.get("h1_rsi_label", "unavailable"),
                "bias": ema_state.get("h1_rsi_signal", "unavailable"),
                "source": ema_state.get("source", source),
                "ok": ema_state.get("h1_rsi") is not None,
            }
        ok = price is not None and source != SOURCE_UNAVAILABLE
        live_health[pair] = {
            "ok": bool(ok),
            "price": price,
            "source": source,
            "ts": ts_iso if ok else None,
            "age_sec": 0 if ok else None,
            "max_age_sec": LIVE_PRICE_MAX_AGE_SEC,
            "published_to_macro_components": False,
            "published_to_rates": False,
            "reason": "ok" if ok else "missing_live_price",
        }
        if ok:
            price_current[pair] = float(price)
            price_current_ts[pair] = ts_iso
            price_current_source[pair] = source
            price_session_ohlc[pair] = {"close": float(price), "_date": ts_iso[:10], "ts": ts_iso, "source": source, "live_close_only": True}
    ...
    patch = {
        ...
        "price_current_ts": price_current_ts,
        "_price_current_ts": price_current_ts,
        "price_current_source": price_current_source,
        "_price_current_source": price_current_source,
        "price_session_ohlc": price_session_ohlc,
        "_price_session_ohlc": price_session_ohlc,
        "h1_atr": h1_atr,
        "h1_atr_proxy": h1_atr,
        "h1_atr_period": H1_ATR_PERIOD,
        "h1_ema_20_50_state": h1_ema_state,
        "h1_rsi_state": h1_rsi_state,
        "live_price_health": live_health,
        "sources": {"live_price": "momentum.py", "rates_json": str(RATES_FILE), "live_context_file": str(LIVE_CONTEXT_FILE)},
    }
    macro_ok = _atomic_merge_json(LIVE_CONTEXT_FILE, patch)
    rates_ok = _publish_rates_json(pairs, current_prices, current_sources, ts_iso)
    for pair in pairs:
        live_health[pair]["published_to_macro_components"] = bool(macro_ok and pair in price_current)
        live_health[pair]["published_to_rates"] = bool(rates_ok and pair in price_current)
    if macro_ok:
        _atomic_merge_json(LIVE_CONTEXT_FILE, {"live_price_health": live_health})
    return live_health
...
def _load_pivot_levels_for_unified_alerts() -> Dict[str, Any]:
    """Load pivot_levels.json written by pivot.py without making API calls."""
    candidates: List[Path] = [PIVOT_LEVELS_FILE]
    try:
        if PIVOT_LEVELS_FILE.name != "pivot_levels.json":
            candidates.append(BASE_DIR / "pivot_levels.json")
    except Exception:
        pass
    candidates.extend([Path("public_html/pivot_levels.json"), Path("public/pivot_levels.json"), Path("pivot_levels.json")])
    seen: Set[str] = set()
    for candidate in candidates:
        try:
            ...
            log.debug("momentum: unified alert pivot load skipped path=%s err=%s", candidate, exc)
    return {}


def _fmt_unified_price(pair: str, value: Any) -> str:
    rounded = _round_price(pair, value)
    if rounded is None:
        return "--"
    return f"{rounded:.2f}" if "xau" in _normalize_pair_code(pair) else f"{rounded:.5f}"


def _fmt_unified_pips(pair: str, value: Any) -> str:
    try:
        x = abs(float(value))
    except (TypeError, ValueError):
        return "--"
    mult = 10 if "xau" in _normalize_pair_code(pair) else 10000
    return f"{x * mult:.1f}p"


def _unified_direction_from_momentum(ema_state: Dict[str, Any]) -> str:
    trend = str(ema_state.get("trend_bias", "")).lower()
    price_vs_fast = str(ema_state.get("price_vs_fast") or ema_state.get("current_vs_ema20") or "").lower()
    price_vs_slow = str(ema_state.get("price_vs_slow") or ema_state.get("current_vs_ema50") or "").lower()
    if "bullish" in trend and price_vs_fast == "above" and price_vs_slow == "above":
        return "bullish"
    if "bearish" in trend and price_vs_fast == "below" and price_vs_slow == "below":
        return "bearish"
    return "neutral"


def _unified_direction_from_pivot(pivot_state: Dict[str, Any]) -> str:
    state = str(pivot_state.get("price_state", "")).lower()
    if state in ("breakout_up", "accept_above_pp"):
        return "bullish"
    if state in ("breakout_down", "accept_below_pp"):
        return "bearish"
    rej = str(pivot_state.get("rejection_level", "")).upper()
    if rej.startswith("S"):
        return "bullish"
    if rej.startswith("R"):
        return "bearish"
...
def _unified_trade_plan_quality_ok(plan: Dict[str, Any]) -> bool:
    ...
    if isinstance(rr, dict):
        try:
            ...
            if best is None:
                return True
            return float(best) >= UNIFIED_EXECUTION_MIN_RR
        except (TypeError, ValueError):
            return True
    return True


def build_unified_execution_alert(pair: str, ema_state: Dict[str, Any], pivot_state: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """Build one institutional-style execution alert when H1 momentum and live pivot state align."""
    pair = _normalize_pair_code(pair)
    if not UNIFIED_EXECUTION_ALERTS_ENABLED:
        return None
    if not isinstance(ema_state, dict) or not ema_state.get("ok"):
        return None
    if not isinstance(pivot_state, dict) or not pivot_state:
        return None

    pivot_basis = str(pivot_state.get("state_basis", "")).lower()
    pivot_reason = str(pivot_state.get("reason", "")).lower()
    pivot_price_state = str(pivot_state.get("price_state", "")).lower()
    if UNIFIED_EXECUTION_REQUIRE_PIVOT_LIVE and pivot_basis != "live":
        return None
    if pivot_price_state in ("", "unavailable") or "missing_live_price" in pivot_reason:
        return None

    mom_dir = _unified_direction_from_momentum(ema_state)
    piv_dir = _unified_direction_from_pivot(pivot_state)
    if mom_dir == "neutral" or piv_dir == "neutral" or mom_dir != piv_dir:
        return None

    atr_conf = pivot_state.get("atr_confirmation") if isinstance(pivot_state.get("atr_confirmation"), dict) else {}
    if UNIFIED_EXECUTION_REQUIRE_ATR_CONFIRMED and atr_conf.get("confirmed") is not True:
        return None
    plan = pivot_state.get("trade_plan") if isinstance(pivot_state.get("trade_plan"), dict) else {}
    if not _unified_trade_plan_quality_ok(plan):
        return None

    direction_icon = "🟢" if mom_dir == "bullish" else "🔴"
    direction_word = "Bullish" if mom_dir == "bullish" else "Bearish"
    arrow = "🔼" if mom_dir == "bullish" else "🔽"
    header = _h1_pair_header(pair, bias=f"{mom_dir}_bias")
    bucket = str(ema_state.get("last_completed_hour", ""))
    ts = _h1_fmt_bucket(bucket)
    state_label = pivot_price_state.replace("_", " ").replace("pp", "PP").title()
    rsi_label = str(ema_state.get("h1_rsi_label", "unavailable")).replace("_", " ")
    h1_atr = ema_state.get("h1_atr") or ema_state.get("h1_atr_proxy")
    atr_reason = str(atr_conf.get("reason", "not_available")).replace("_", " ") if atr_conf else "not available"
    atr_status = "confirmed" if atr_conf.get("confirmed") is True else "unconfirmed" if atr_conf.get("confirmed") is False else "not available"

    current = pivot_state.get("price_used") or ema_state.get("price")
    pp, r1, r2, s1, s2 = [pivot_state.get(k) for k in ("PP", "R1", "R2", "S1", "S2")]
    entry = plan.get("entry") if isinstance(plan, dict) else None
    zone = plan.get("entry_zone") if isinstance(plan, dict) else None
    sl = plan.get("stop_loss") if isinstance(plan, dict) else None
    tp = plan.get("take_profit") if isinstance(plan, dict) else []
    rr = plan.get("risk_reward") if isinstance(plan, dict) else {}
    rr_best = rr.get("best") if isinstance(rr, dict) else None
    action = str(plan.get("action", "observe")).replace("_", " ") if isinstance(plan, dict) else "observe"
    invalidation = str(plan.get("invalidation", "Momentum/pivot alignment fails")) if isinstance(plan, dict) else "Momentum/pivot alignment fails"
    zone_text = "--"
    if isinstance(zone, dict):
        zone_text = f"{_fmt_unified_price(pair, zone.get('low'))}–{_fmt_unified_price(pair, zone.get('high'))}"
    tp_text = " / ".join(_fmt_unified_price(pair, x) for x in tp) if isinstance(tp, list) and tp else "--"
    rr_text = f"{float(rr_best):.2f}" if rr_best is not None else "--"

    if pivot_price_state == "breakout_up":
        execution_idea = "Bullish expansion above R1. Watch R1/EMA20 retest for continuation toward R2."
    elif pivot_price_state == "breakout_down":
        execution_idea = "Bearish expansion below S1. Watch S1/EMA20 retest for continuation toward S2."
    elif pivot_price_state == "accept_above_pp":
        execution_idea = "Bullish acceptance above PP. Watch PP/EMA20 support for continuation toward R1."
    elif pivot_price_state == "accept_below_pp":
        execution_idea = "Bearish acceptance below PP. Watch PP/EMA20 resistance for continuation toward S1."
    else:
        execution_idea = f"{direction_word} continuation while momentum and pivot structure remain aligned."

    text = "\n".join([
        f"<b>🧠 FX EXECUTION SIGNAL | {header} {arrow}</b>",
        "",
        "<b>Market State</b>",
        f"• Bias: {direction_icon} {direction_word} EMA alignment",
        f"• Pivot: {state_label}",
        f"• Momentum: H1 RSI {rsi_label}",
        f"• Candle: {ts}",
        "",
        "<b>Price / Levels</b>",
        f"• Current: {_fmt_unified_price(pair, current)}",
        f"• PP: {_fmt_unified_price(pair, pp)}",
        f"• R1/R2: {_fmt_unified_price(pair, r1)} / {_fmt_unified_price(pair, r2)}",
        f"• S1/S2: {_fmt_unified_price(pair, s1)} / {_fmt_unified_price(pair, s2)}",
        "",
        "<b>Volatility / Confirmation</b>",
        f"• H1 ATR proxy: {_fmt_unified_pips(pair, h1_atr)}",
        f"• Pivot ATR: {atr_status} ({atr_reason})",
        "",
        "<b>Execution Plan</b>",
        f"• Action: {action}",
        f"• Entry: {_fmt_unified_price(pair, entry)}",
        f"• Zone: {zone_text}",
        f"• SL: {_fmt_unified_price(pair, sl)}",
        f"• TP: {tp_text}",
        f"• RR best: {rr_text}",
        "",
        "<b>Execution Idea</b>",
        ...
        "• No entry if live price or momentum alignment disappears.",
        "─────────────────────",
        "<b>🧩 Context:</b> Momentum + live pivot structure aligned.",
    ])
    key_bucket = bucket or _iso_z(_hour_bucket(_now_utc()))
    key = f"{pair}:unified:{mom_dir}:{pivot_price_state}:{key_bucket}"
    return {"key": key, "text": text, "level": "unified", "direction": mom_dir}


def dispatch_unified_execution_alerts(
    ema_states: Dict[str, Dict[str, Any]],
    bot_token: str = TELEGRAM_BOT_TOKEN,
    chat_id: int = TELEGRAM_CHAT_ID,
    dry_run: bool = False,
) -> int:
    """Send unified momentum+pivot execution alerts with persistent de-duplication."""
    if not UNIFIED_EXECUTION_ALERTS_ENABLED:
        return 0
    if not bot_token:
        log.debug("momentum: Telegram bot token not configured — unified alerts skipped")
        return 0
    if not dry_run and not chat_id:
        log.warning("momentum: TELEGRAM_CHAT_ID not set — unified alerts disabled")
        return 0
    pivot_levels = _load_pivot_levels_for_unified_alerts()
    pivot_pairs = pivot_levels.get("pairs", {}) if isinstance(pivot_levels, dict) else {}
    if not isinstance(pivot_pairs, dict) or not pivot_pairs:
        log.debug("momentum: unified alerts skipped — no pivot_levels.json pairs")
        return 0
    state = _h1_prune_alert_state(_h1_load_alert_state(), max_age_hours=UNIFIED_EXECUTION_ALERT_TTL_HOURS)
    sent = 0
    for pair, ema_state in ema_states.items():
        pair_l = _normalize_pair_code(pair)
        pivot_state = pivot_pairs.get(pair_l) or pivot_pairs.get(pair_l.upper()) or {}
        alert = build_unified_execution_alert(pair_l, ema_state, pivot_state)
        if not alert:
            continue
        key = alert["key"]
        if key in state:
            log.debug("momentum: skipping duplicate unified alert key=%s", key)
            continue
        if dry_run:
            log.info("[DRY-RUN] Would send unified execution alert key=%s:\n%s", key, alert["text"])
            sent += 1
        else:
            ok = _tg_send(bot_token, chat_id, alert["text"])
            if ok:
                state[key] = _iso_z(_now_utc())
                sent += 1
                log.info("momentum: sent unified %s execution alert for %s (key=%s)", alert.get("direction"), pair_l, key)
            else:
                log.warning("momentum: failed to send unified execution alert for %s", pair_l)
    if not dry_run:
        _h1_save_alert_state(state)
    return sent
...
def fetch_price_momentum(
    ...
    for pair in pair_list:
        ...
        _h1_atr_val = _h1_atr_proxy_from_rows(rows, H1_ATR_PERIOD)
        diagnostics[pair] = {
            "score": score,
            ...
            "h1_atr": float(_h1_atr_val) if _h1_atr_val is not None else 0.0,
            "h1_atr_period": float(H1_ATR_PERIOD),
            "history_source": src,
            "repair_note": note,
    ...
    for pair in pair_list:
        ...
        _h1_atr_state = _h1_atr_proxy_from_rows(histories.get(pair, []), H1_ATR_PERIOD)
        _state["h1_atr"] = round(float(_h1_atr_state), _price_digits(pair)) if _h1_atr_state is not None else None
        _state["h1_atr_proxy"] = _state["h1_atr"]
        _state["h1_atr_period"] = H1_ATR_PERIOD
    ...
    # Publish live prices + H1 context for pivot.py and pivots.html.
    live_health = _publish_live_context(pair_list, histories, current_prices, current_sources, _LAST_H1_EMA_STATE)
    for _pair, _health in live_health.items():
        diagnostics.setdefault(_pair, {})["live_price_published"] = bool(_health.get("published_to_macro_components"))
        diagnostics.setdefault(_pair, {})["rates_published"] = bool(_health.get("published_to_rates"))
        diagnostics.setdefault(_pair, {})["live_price_health_ok"] = bool(_health.get("ok"))
    _LAST_MOMENTUM_DIAGNOSTICS.clear()
    _LAST_MOMENTUM_DIAGNOSTICS.update(diagnostics)

    # Dispatch Hourly EMA 20/50 Telegram alerts and unified execution alerts.
    if TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID:
        ...
        try:
            dispatch_unified_execution_alerts(_LAST_H1_EMA_STATE)
        except Exception as _ue_exc:
            log.warning("momentum: unified execution alert dispatch failed: %s", _ue_exc)
