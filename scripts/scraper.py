...
def load_config() -> Dict:
    ...
    try:
        ...
        # Warn on threshold keys that don't match any active _STOOQ_PAIRS label
        # so stale overrides (e.g. old pair names) don't silently go unused.
        known_labels = {label for label, _, _ in _STOOQ_PAIRS}
        for k in cfg.get("rates_change_thresholds", {}):
            if k != "_default" and k not in known_labels:
                logging.getLogger(__name__).warning(
                    "Config: rates_change_thresholds key %r doesn't match any "
                    "active pair %s -- entry will be ignored",
                    k, sorted(known_labels),
                )
...
def setup_logging(log_file: str, log_retention_days: int = 0,
    ...
    log_to_stdout = bool(cfg.get("log_to_stdout", False)) if cfg is not None else False
    if log_to_stdout:
    ...
    try:
        ...
        if log_to_stdout:
            ...
            stdout_h.setFormatter(fmt)
            logger.addHandler(stdout_h)
            logger.warning(f"Cannot open log file {log_file}: {exc} -- stdout fallback enabled")

    if trim_msg:
        (logger.warning if "failed" in trim_msg else logger.info)(trim_msg)

    logger.info("Logging configured -- stdout=%s file=%s propagate=%s", log_to_stdout, log_file, logger.propagate)
...
# ---------------------------------------------------------------------------
# Stooq sliding-window rate limiter
# Enforces a hard cap of max_per_min requests per 60-second rolling window
# across all threads.  Complements _throttle() (per-domain last-hit spacing)
# with an absolute ceiling that prevents burst-firing when several pairs
# refresh simultaneously after a long cache hit streak.
# ---------------------------------------------------------------------------
_STOOQ_REQ_TIMES: List[float] = []
_STOOQ_REQ_LOCK  = threading.Lock()

def _stooq_rate_limit(max_per_min: int = 8) -> None:
    now = time.time()
    with _STOOQ_REQ_LOCK:
        # Evict timestamps older than 60 s
        _STOOQ_REQ_TIMES[:] = [t for t in _STOOQ_REQ_TIMES if now - t < 60.0]
        if len(_STOOQ_REQ_TIMES) >= max_per_min:
            wait = 60.0 - (now - _STOOQ_REQ_TIMES[0]) + random.uniform(0.2, 0.6)
            if wait > 0:
                logging.getLogger(__name__).debug(
                    "Stooq rate-limit: window full (%d/%d) -- sleeping %.1fs",
                    len(_STOOQ_REQ_TIMES), max_per_min, wait,
                )
                time.sleep(wait)
        _STOOQ_REQ_TIMES.append(time.time())
...
# ---------------------------------------------------------------------------
# Circuit breaker
# After _CIRCUIT_FAIL_THRESHOLD consecutive failures within _CIRCUIT_WINDOW_S
# the source is skipped until it cools off.  Prevents one slow/dead endpoint
# from monopolising the thread pool timeout budget every run.
# ---------------------------------------------------------------------------
_CIRCUIT_FAIL_THRESHOLD: int   = 3
_CIRCUIT_WINDOW_S:        float = 3600.0   # 1 hour cool-off
...
def _circuit_is_open(name: str) -> bool:
    with _CIRCUIT_LOCK:
        ...
        if not state:
            return False
        return (
            state["fails"] >= _CIRCUIT_FAIL_THRESHOLD
            and time.time() - state["last_ok"] < _CIRCUIT_WINDOW_S
        )

def _circuit_record_ok(name: str) -> None:
    with _CIRCUIT_LOCK:
        _CIRCUIT_STATE[name] = {"fails": 0, "last_ok": time.time()}

def _circuit_record_fail(name: str) -> None:
    with _CIRCUIT_LOCK:
        state = _CIRCUIT_STATE.setdefault(name, {"fails": 0, "last_ok": 0.0})
        state["fails"] += 1
...
class SourceHandler(ABC):
    ...
    def run(self, scrape_time: str) -> List[Dict]:
        if _circuit_is_open(self.name):
            self.log.info(
                "[%s] circuit open (%d consecutive failures) -- skipping this run",
                self.name, _CIRCUIT_STATE.get(self.name, {}).get("fails", 0),
            )
            self.status.record(self.name, success=False,
                               error="circuit_open", duration=0.0)
            return []
        t0 = time.monotonic()
        try:
            items    = self.fetch(scrape_time)
            duration = time.monotonic() - t0
            _circuit_record_ok(self.name)
            self.status.record(self.name, success=True, items=len(items),
                               duration=duration)
            return items
        except Exception as exc:
            duration = time.monotonic() - t0
            _circuit_record_fail(self.name)
...
def main() -> None:
    ...
    if "cal" in handler_map:
        ...
        log.info(
            "calendar.json written early (%d events) -- macro.py will enrich PMI via StooqTicker",
            len(calendar_events),
    ...
    if remaining_handlers:
        with ThreadPoolExecutor(max_workers=min(max(1, len(remaining_handlers)), 5)) as ex:
