#!/usr/bin/env python3
# -----------------------------------------------------------------------------
# LIVE-ONLY PIVOT SIGNAL BUILD
# Generated by M365 Copilot on 2026-05-20.
# Final fix: pivot signals never fall back to prior_close. If current_price is
# unavailable, the signal is marked unavailable with reason=missing_live_price.
# ATR confirmation added: weak breakout/accept moves can be gated by ATR5.
# Pivot-zone entry/SL/TP levels added for educational planning output.
# -----------------------------------------------------------------------------
...
# ── ATR confirmation settings ────────────────────────────────────────────────
# The pivot state is still calculated from live price only. ATR confirmation adds
# a volatility-aware minimum movement requirement before breakout/accept states
# are allowed to stand. If a candidate move is too small relative to ATR, the
# state is downgraded to a less aggressive live state instead of producing a
# weak signal.
ATR_CONFIRM_ENABLED: bool = os.environ.get("PRICE_ATR_CONFIRM_ENABLED", "1").strip().lower() not in ("0", "false", "no", "off")
ATR_CONFIRM_GATE_STATE: bool = os.environ.get("PRICE_ATR_CONFIRM_GATE_STATE", "1").strip().lower() not in ("0", "false", "no", "off")
ATR_CONFIRM_REQUIRE_AVAILABLE: bool = os.environ.get("PRICE_ATR_CONFIRM_REQUIRE_AVAILABLE", "0").strip().lower() in ("1", "true", "yes", "on")
ATR_CONFIRM_BREAKOUT_MULT: float = _env_float("PRICE_ATR_CONFIRM_BREAKOUT_MULT", 0.10, minimum=0.0)
ATR_CONFIRM_ACCEPT_MULT: float = _env_float("PRICE_ATR_CONFIRM_ACCEPT_MULT", 0.05, minimum=0.0)
ATR_CONFIRM_REJECTION_MULT: float = _env_float("PRICE_ATR_CONFIRM_REJECTION_MULT", 0.05, minimum=0.0)

# ── Pivot-zone trade plan settings ───────────────────────────────────────────
# Educational levels only: these fields describe pivot-zone structure derived
# from the live signal. They are not trade commands and should be combined with
# execution rules, spread/slippage checks, and risk controls outside this module.
PIVOT_TRADE_PLAN_ENABLED: bool = os.environ.get("PRICE_PIVOT_TRADE_PLAN_ENABLED", "1").strip().lower() not in ("0", "false", "no", "off")
PIVOT_ENTRY_ZONE_ATR_MULT: float = _env_float("PRICE_PIVOT_ENTRY_ZONE_ATR_MULT", 0.05, minimum=0.0)
PIVOT_ENTRY_ZONE_RANGE_MULT: float = _env_float("PRICE_PIVOT_ENTRY_ZONE_RANGE_MULT", 0.02, minimum=0.0)
PIVOT_SL_ATR_MULT: float = _env_float("PRICE_PIVOT_SL_ATR_MULT", 0.15, minimum=0.0)
PIVOT_SL_RANGE_MULT: float = _env_float("PRICE_PIVOT_SL_RANGE_MULT", 0.03, minimum=0.0)
PIVOT_MIN_RR: float = _env_float("PRICE_PIVOT_MIN_RR", 1.20, minimum=0.0)
...
def _fetch_components_ohlc(
    ...
    # Live-only signal rule: never derive current_price from the daily OHLC close.
    # Prefer explicit price_current, then scraper session close. If neither exists,
    # leave current_price unset so the signal is marked unavailable instead of
    # silently falling back to prior close.
    _attach("current_price", current_map.get(pair_l), session_dict.get("close"))
    ...
    if not PIVOT_FORCE_REFRESH:
        ...
        if cached and cached.get("session_date") == session_date:
            ...
            if isinstance(raw_ohlc, dict):
                ...
                if ohlc is not None:
                    # Live-only signal rule: _clean_ohlc intentionally strips
                    # non-OHLC fields. Restore live context so classification
                    # uses the latest price whenever it is available.
                    def _copy_live_fields(src: object) -> bool:
                        if not isinstance(src, dict):
                            return False
                        copied = False
                        for key in ("current_price", "atr5", "session_high", "session_low", "rsi", "rsi_period"):
                            value = src.get(key)
                            if value is None:
                                continue
                            try:
                    ...
                    _copy_live_fields(raw_ohlc)

                    # Overlay the freshest live context from macro_components.json
                    # while keeping the cached daily OHLC for pivot levels.
                    # This prevents a valid daily-pivot cache from forcing a
                    # prior-close signal during intraday moves.
                    if _components_are_fresh(component_file):
                        fresh_context = _fetch_components_ohlc(pair_l, component_file)
                        if _copy_live_fields(fresh_context):
                            cached_source = f"{cached_source}+live_components"

                    log.info(
                        "pivot[%s]: SQLite OHLC cache hit session=%s source=%s live_price=%s",
                        pair_l, session_date, cached_source, ohlc.get("current_price"),
                    )
...
def classify_price_structure(
    ...
    # FINAL FIX: signals are strictly live-only.
    # Never classify from pivots["close"] / prior close. If no live price is
    # available, return an unavailable result so downstream code cannot emit a
    # misleading prior-close signal.
    if current_price is None:
        unavailable = _unavailable_result(macro_score)
        unavailable.update({
            "reason": "live_price_required",
            "price_state": STATE_UNAVAILABLE,
            ...
            "range": pivots.get("range", 0.0),
            "close": pivots.get("close", 0.0),
        })
        return unavailable
    ...
    try:
        if atr5 is not None and float(atr5) > 0:
            atr_val = float(atr5)
    except (TypeError, ValueError):
        atr_val = None

    def _atr_confirmation_payload(
        candidate_state: str,
        final_state: str,
        ...
        return {
            "enabled": ATR_CONFIRM_ENABLED,
            "available": atr_val is not None,
            "confirmed": confirmed,
            "candidate_state": candidate_state,
            "final_state": final_state,
            "actual_move": round(max(actual_move, 0.0), 6),
            "required_move": round(max(required_move, 0.0), 6),
            "atr5": round(atr_val, 6) if atr_val is not None else None,
            "basis": basis,
            "reason": reason,
        }

    def _downgrade_unconfirmed(candidate_state: str) -> str:
        # Keep the signal live, but reduce aggressiveness when the move is too
        # small relative to ATR. This avoids calling tiny pierces a confirmed
        # breakout while still preserving above/below PP acceptance context.
        if candidate_state == "breakout_up":
            return "accept_above_pp" if c > pp + bal else "balance"
        if candidate_state == "breakout_down":
            return "accept_below_pp" if c < pp - bal else "balance"
        if candidate_state in ("accept_above_pp", "accept_below_pp"):
            return "balance"
    ...
    atr_actual_move = 0.0
    atr_required_move = 0.0
    atr_basis = "not_required"
    atr_confirmed: Optional[bool] = True
    atr_reason = "atr_confirmation_disabled" if not ATR_CONFIRM_ENABLED else "not_required_for_state"

    if ATR_CONFIRM_ENABLED and candidate_state in (
        "breakout_up", "breakout_down", "accept_above_pp", "accept_below_pp", "rejection"
    ):
        if atr_val is None:
            atr_confirmed = None
            atr_reason = "atr_unavailable"
            atr_basis = "atr5_missing"
            if ATR_CONFIRM_REQUIRE_AVAILABLE and ATR_CONFIRM_GATE_STATE:
                state = _downgrade_unconfirmed(candidate_state)
                atr_confirmed = False
                atr_reason = "atr_required_but_unavailable"
        else:
            if candidate_state == "breakout_up":
                atr_actual_move = c - r1
                atr_required_move = max(tol, atr_val * ATR_CONFIRM_BREAKOUT_MULT)
                atr_basis = "live_price_minus_R1"
            elif candidate_state == "breakout_down":
                atr_actual_move = s1 - c
                atr_required_move = max(tol, atr_val * ATR_CONFIRM_BREAKOUT_MULT)
                atr_basis = "S1_minus_live_price"
            elif candidate_state == "accept_above_pp":
                atr_actual_move = c - pp
                atr_required_move = max(bal, atr_val * ATR_CONFIRM_ACCEPT_MULT)
                atr_basis = "live_price_minus_PP"
            elif candidate_state == "accept_below_pp":
                atr_actual_move = pp - c
                atr_required_move = max(bal, atr_val * ATR_CONFIRM_ACCEPT_MULT)
                atr_basis = "PP_minus_live_price"
            elif candidate_state == "rejection":
                if rejection_level.startswith("R"):
                    atr_actual_move = max(session_high or c, c) - c
                elif rejection_level.startswith("S"):
                    atr_actual_move = c - min(session_low or c, c)
                atr_required_move = atr_val * ATR_CONFIRM_REJECTION_MULT
                atr_basis = f"pullback_from_{rejection_level}"

            atr_confirmed = atr_actual_move >= atr_required_move
            atr_reason = "confirmed_by_atr" if atr_confirmed else "move_too_small_vs_atr"
            if not atr_confirmed and ATR_CONFIRM_GATE_STATE:
                state = _downgrade_unconfirmed(candidate_state)
    ...
    if has_current and has_rejection_evidence and atr_confirmed is True:
        state_quality = "high"
    elif has_current and atr_confirmed is not False:
    ...
    def _fmt_level(value: Optional[float]) -> Optional[float]:
        if value is None:
            return None
        try:
    ...
    def _rr(direction: str, entry: Optional[float], sl: Optional[float], tp: Optional[float]) -> Optional[float]:
        if entry is None or sl is None or tp is None:
            return None
        risk = abs(float(entry) - float(sl))
        if risk <= 0:
            return None
        reward = (float(tp) - float(entry)) if direction == "long" else (float(entry) - float(tp))
        if reward <= 0:
            return None
        return round(reward / risk, 2)

    def _zone(center: float, side: str, width: float) -> Dict[str, float]:
        # side="above" gives a buy/retest zone above the pivot; side="below"
        # gives a sell/retest zone below the pivot; side="around" straddles it.
        if side == "above":
            lo, hi = center, center + width
        elif side == "below":
            lo, hi = center - width, center
        else:
            lo, hi = center - width, center + width
        return {"low": round(min(lo, hi), 6), "high": round(max(lo, hi), 6)}

    def _build_pivot_trade_plan() -> Dict[str, Union[str, float, bool, None, Dict[str, float], List[float]]]:
        if not PIVOT_TRADE_PLAN_ENABLED:
            return {"enabled": False, "action": "disabled", "reason": "trade_plan_disabled"}

        # Use ATR when available; otherwise fall back to a conservative fraction
        # of the prior daily pivot range so the plan still has structure.
        zone_width = (atr_val * PIVOT_ENTRY_ZONE_ATR_MULT) if atr_val is not None else (rng * PIVOT_ENTRY_ZONE_RANGE_MULT)
        sl_buffer = (atr_val * PIVOT_SL_ATR_MULT) if atr_val is not None else (rng * PIVOT_SL_RANGE_MULT)
        zone_width = max(zone_width, tol, 0.0)
        sl_buffer = max(sl_buffer, tol, 0.0)

        direction: str = "none"
        action: str = "observe"
        setup: str = state
        entry_ref: Optional[float] = None
        zone_side: str = "around"
        sl: Optional[float] = None
        tp_values: List[float] = []
        invalidation: str = ""
        reason: str = "state_has_no_trade_plan"

        if state == "breakout_up":
            direction, action, entry_ref, zone_side = "long", "buy_retest", r1, "above"
            sl = r1 - sl_buffer
            tp_values = [r2, r3]
            invalidation = "live price loses R1 after breakout"
            reason = "breakout_up_plan_from_R1_to_R2_R3"
        elif state == "breakout_down":
            direction, action, entry_ref, zone_side = "short", "sell_retest", s1, "below"
            sl = s1 + sl_buffer
            tp_values = [s2, s3]
            invalidation = "live price reclaims S1 after breakdown"
            reason = "breakout_down_plan_from_S1_to_S2_S3"
        elif state == "accept_above_pp":
            direction, action, entry_ref, zone_side = "long", "buy_pp_retest", pp, "above"
            sl = pp - sl_buffer
            tp_values = [r1, r2]
            invalidation = "live price accepts back below PP"
            reason = "accept_above_pp_plan_from_PP_to_R1_R2"
        elif state == "accept_below_pp":
            direction, action, entry_ref, zone_side = "short", "sell_pp_retest", pp, "below"
            sl = pp + sl_buffer
            tp_values = [s1, s2]
            invalidation = "live price accepts back above PP"
            reason = "accept_below_pp_plan_from_PP_to_S1_S2"
        elif state == "rejection" and rejection_level:
            level_map = {"R3": r3, "R2": r2, "R1": r1, "PP": pp, "S1": s1, "S2": s2, "S3": s3}
            level_order = ["R3", "R2", "R1", "PP", "S1", "S2", "S3"]
            idx = level_order.index(rejection_level) if rejection_level in level_order else -1
            if rejection_level.startswith("R") and idx >= 0:
                direction, action, entry_ref, zone_side = "short", "sell_rejection", level_map[rejection_level], "below"
                sl = level_map[rejection_level] + sl_buffer
                lower_names = level_order[idx + 1: idx + 3]
                tp_values = [level_map[name] for name in lower_names if name in level_map]
                invalidation = f"live price accepts above {rejection_level}"
                reason = f"resistance_rejection_plan_from_{rejection_level}"
            elif rejection_level.startswith("S") and idx >= 0:
                direction, action, entry_ref, zone_side = "long", "buy_rejection", level_map[rejection_level], "above"
                sl = level_map[rejection_level] - sl_buffer
                upper_names = list(reversed(level_order[max(0, idx - 2): idx]))
                tp_values = [level_map[name] for name in upper_names if name in level_map]
                invalidation = f"live price accepts below {rejection_level}"
                reason = f"support_rejection_plan_from_{rejection_level}"
        elif state == "balance":
            return {
                "enabled": True,
                "action": "observe",
                "direction": "none",
                "setup": "balance",
                "reason": "price_inside_balance_zone_no_entry_sl_tp",
                "entry": None,
                "entry_zone": None,
                "stop_loss": None,
                "take_profit": [],
                "risk_reward": None,
                "min_rr": round(PIVOT_MIN_RR, 2),
                "quality": "none",
            }

        if direction == "none" or entry_ref is None or sl is None or not tp_values:
            return {
                "enabled": True,
                "action": "observe",
                "direction": "none",
        ...
        entry_zone = _zone(entry_ref, zone_side, zone_width)
        # Use midpoint of the zone for RR estimation only; actual execution can
        # choose limit/market rules elsewhere.
        entry_mid = (entry_zone["low"] + entry_zone["high"]) / 2.0
        rr1 = _rr(direction, entry_mid, sl, tp_values[0] if len(tp_values) > 0 else None)
        rr2 = _rr(direction, entry_mid, sl, tp_values[1] if len(tp_values) > 1 else None)
        best_rr = max([x for x in (rr1, rr2) if x is not None], default=None)
        quality = "valid" if best_rr is not None and best_rr >= PIVOT_MIN_RR else "low_rr"
        if atr_confirmed is False:
            quality = "atr_unconfirmed"
        ...
        return {
            ...
            "setup": setup,
            "reason": reason,
            "entry": _fmt_level(entry_ref),
            "entry_zone": entry_zone,
            "entry_reference": _fmt_level(entry_ref),
            "stop_loss": _fmt_level(sl),
            "take_profit": [_fmt_level(v) for v in tp_values if _fmt_level(v) is not None],
            "tp1": _fmt_level(tp_values[0]) if len(tp_values) > 0 else None,
            "tp2": _fmt_level(tp_values[1]) if len(tp_values) > 1 else None,
            "risk_reward": {"tp1": rr1, "tp2": rr2, "best": best_rr},
            "min_rr": round(PIVOT_MIN_RR, 2),
            "quality": quality,
            "zone_width": round(zone_width, 6),
            "sl_buffer": round(sl_buffer, 6),
            "invalidation": invalidation,
            "basis": "live_price_pivot_zone_atr" if atr_val is not None else "live_price_pivot_zone_range_fallback",
            "note": "Educational pivot-zone levels only; not a trade command.",
        }

    trade_plan = _build_pivot_trade_plan()
    ...
    return {
        ...
        "atr_confirmation": _atr_confirmation_payload(
            candidate_state=candidate_state,
            final_state=state,
            actual_move=atr_actual_move,
            required_move=atr_required_move,
            confirmed=atr_confirmed,
            basis=atr_basis,
            reason=atr_reason,
        ),
        "atr_confirmed": atr_confirmed,
        "atr_required_move": round(max(atr_required_move, 0.0), 6),
        "atr_actual_move": round(max(atr_actual_move, 0.0), 6),
        "trade_plan": trade_plan,
        "entry": trade_plan.get("entry") if isinstance(trade_plan, dict) else None,
        "entry_zone": trade_plan.get("entry_zone") if isinstance(trade_plan, dict) else None,
        "stop_loss": trade_plan.get("stop_loss") if isinstance(trade_plan, dict) else None,
        "take_profit": trade_plan.get("take_profit") if isinstance(trade_plan, dict) else [],
        "tp1": trade_plan.get("tp1") if isinstance(trade_plan, dict) else None,
        "tp2": trade_plan.get("tp2") if isinstance(trade_plan, dict) else None,
        "risk_reward": trade_plan.get("risk_reward") if isinstance(trade_plan, dict) else None,
...
def fetch_price_structure(
    ...
    for pair in pairs:
        ...
        # FINAL FIX: hard block prior-close signals.
        # If no live current_price is available, do not call the classifier.
        if live["current_price"] is None:
            ...
            result.update({
                ...
                "close": pivots.get("close", 0.0),
                "ohlc_source": source,
                "ohlc_date": ohlc.get("_date", ""),
            })
            results[pair_l] = result
            sources[pair_l] = "no_live_price"
            continue
